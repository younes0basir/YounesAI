const NVIDIA_IMAGE_API_URL = process.env.NVIDIA_IMAGE_API_URL || 'https://ai.api.nvidia.com/v1/genai/black-forest-labs/flux.2-klein-4b'

function buildNvidiaAuthError(detail) {
  return new Error(
    'NVIDIA image generation authorization failed. Check the NVIDIA_IMAGE_API_KEY or NVIDIA_API_KEY in backend/.env and regenerate it if it is expired or invalid.'
  ...
  for (const candidate of candidates) {
    ...
    if (Array.isArray(candidate)) {
      for (const item of candidate) {
        ...
        const value = item.base64 || item.b64 || item.image_base64 || item.image || item.url || item.image_url
    ...
    const value = candidate.base64 || candidate.b64 || candidate.image_base64 || candidate.image || candidate.url || candidate.image_url
    if (value) return value
  }
...
async function generateImage({ prompt, width = 768, height = 1344, steps = 4, seed = 1 }) {
  ...
  if (!apiKey) {
    throw new Error('NVIDIA_IMAGE_API_KEY is not configured. Add it to your backend .env file.')
  }
