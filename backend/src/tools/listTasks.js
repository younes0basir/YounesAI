...
async function listTasks(context, filters = {}) {
  ...
  if (filters.priority) { where.push(`priority = $${idx++}`); params.push(Number(filters.priority)); }
  if (filters.filter === 'today') { where.push('due_at IS NOT NULL AND DATE(due_at) = CURRENT_DATE'); }
  if (filters.filter === 'overdue') { where.push("status != 'done' AND due_at IS NOT NULL AND due_at < NOW()"); }
