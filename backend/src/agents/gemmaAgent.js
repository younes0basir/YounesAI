    ...
    try {
      ...
      if (parsed.action === 'plan') {
        ...
        if (folderContents.length > 0) {
          ...
          return {
            success: true,
            content: prefixWithSourceCheck(planSummary + folderInfo, context, [
              'file planning result',
            ]),
            metadata: { provider: result.provider, model: result.model },
          };
        }
        ...
        return {
          success: true,
          content: prefixWithSourceCheck(planSummary, context, ['file planning result']),
          metadata: { provider: result.provider, model: result.model },
        };
      }
