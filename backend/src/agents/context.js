...
async function buildContext(req) {
  ...
  if (userId && req.body?.folderId) {
    const fRes = await pool.query(
      'SELECT id, folder_path FROM indexed_folders WHERE id = $1 AND user_id = $2',
      [req.body.folderId, userId]
    ).catch(() => ({ rows: [] }));
  ...
  const [recentConversations, relevantDocuments] = await Promise.all([
    userId
      ? retrieveConversations({ userId, limit: 5 }).catch(() => [])
      : Promise.resolve([]),

    userId && req.body?.message
      ? retrieveDocuments({ userId, query: req.body.message, limit: 5, folderPath: activeFolder?.folderPath }).catch(() => [])
      : Promise.resolve([]),
  ]);
...
function buildSystemPromptContext(context) {
  ...
  if (context.parsedParameters?.dueAt) {
    parts.push(`Pre-parsed date: ${context.parsedParameters.dueAt} (use this as the due date for tasks)`);
  }
  if (context.parsedParameters?.startsAt) {
    parts.push(`Pre-parsed start time: ${context.parsedParameters.startsAt} (use this as starts_at for events, ends_at = +1 hour)`);
  }

  if (context.activeFolder) {
    const name = context.activeFolder.folderPath.split(/[\\/]/).pop() || context.activeFolder.folderPath;
...
function buildSourceCheck(context, extraSources = []) {
  ...
  if (context?.activeFolder?.folderPath) {
    const folderName = path.basename(context.activeFolder.folderPath) || context.activeFolder.folderPath;
...
module.exports = { buildContext, buildSystemPromptContext, buildSourceCheck, prefixWithSourceCheck };
