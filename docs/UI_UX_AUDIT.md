...
Tailwind 4 `@theme` tokens (primary = cyan, accent = violet) + a large hand-written layer: surfaces (glassmorphism), buttons, inputs, pills, badges, page-header, rail, agents-*, pipeline-*, sandbox-*, etc. `src/App.css` is leftover Vite-template CSS (unused).
...
1. **B1** — `index.css` is duplicated with conflicting badge colors (live contrast bug). *Fix first.*
