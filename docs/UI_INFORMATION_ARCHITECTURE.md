...
| `/voice` | AI Assistant panel | **Assistant** — demoted to a tool linked *from inside* the Assistant page, not a top-level nav item | route stays live for deep-linking/bookmarks |
...
const railRegistry = [
  ...
  { match: (path) => path.startsWith('/chat')
                   ...
                   || path.startsWith('/image-generator'),  panel: AssistantRailPanel },
