...
*   **Temporal Parser Utility (`src/utils/temporalUtility.js`)**: Intercepts requests marked as time-sensitive. It leverages `chrono-node` to parse relative expressions (e.g., *"in 10 minutes"*) into clean, absolute ISO 8601 strings anchored to the client's current year (2026), and strips the time terms from the prompt before routing downstream.
