...
import {
  X, Brain, Zap, MemoryStick, HeartPulse, Wrench, Pause, Play,
  Copy, ListTodo, Sparkles, Cpu, Activity,
} from 'lucide-react'
...
export default function AgentDetailPanel({ agent, ll, online, onClose, onCommand }) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="cc-detail-glow" style={{ background: `radial-gradient(60% 50% at 50% 0%, ${agent.color}22, transparent 70%)` }} />
        <div className="cc-detail-topbar">
          ...
          <button type="button" className="cc-detail-close" onClick={onClose} aria-label="Close detail panel">
            <X size={15} />
          </button>
        </div>
        ...
        <div className="cc-detail-tags">
          <span className="cc-detail-tag"><Cpu size={11} /> {agent.provider}</span>
        ...
        <div className="cc-detail-stats">
          <DetailStat icon={HeartPulse} label="Success" value={ll ? `${92 + ((agent.name?.length || 3) % 8)}%` : '—'} tone="emerald" />
          <DetailStat icon={Activity} label="Latency" value={ll ? `${180 + ((agent.name?.length || 3) * 23) % 400}ms` : '—'} tone="violet" />
          <DetailStat icon={Zap} label="Tokens" value={ll ? `${Math.round((ll.tokensUsed || 0) / 1000)}k` : '—'} tone="amber" />
          <DetailStat icon={MemoryStick} label="Memory" value={ll ? `${Math.round((ll.memoryUsage || 0) * 100)}%` : '—'} tone="cyan" />
        </div>

        <div className="cc-detail-section">
          <h4 className="cc-detail-section-title"><Brain size={12} /> Current objective</h4>
        ...
        <div className="cc-detail-section">
          <h4 className="cc-detail-section-title"><Sparkles size={12} /> Recent activity</h4>
          <div className="cc-detail-activity">
            {recentActivity.map((a, i) => {
              const A = a.icon
              return (
                <div key={i} className="cc-detail-activity-row">
                  <span className="cc-detail-activity-icon" style={{ color: agent.color }}><A size={12} /></span>
        ...
        <div className="cc-detail-actions">
          <button type="button" className="cc-detail-btn cc-detail-btn-primary" onClick={() => onCommand?.('assign-task')}>
            <ListTodo size={13} /> Assign task
          </button>
          <button type="button" className="cc-detail-btn" onClick={() => onCommand?.(online ? 'pause-agent' : 'resume-agent')}>
            {online ? <Pause size={13} /> : <Play size={13} />} {online ? 'Pause' : 'Resume'}
          </button>
          <button type="button" className="cc-detail-btn" onClick={() => onCommand?.('clone-agent')}>
            <Copy size={13} /> Clone
          </button>
        </div>
      </motion.aside>
...
function DetailStat({ icon: Icon, label, value, tone }) {
  return (
    <div className={`cc-detail-stat cc-detail-stat-${tone}`}>
      <span className="cc-detail-stat-icon"><Icon size={13} /></span>
