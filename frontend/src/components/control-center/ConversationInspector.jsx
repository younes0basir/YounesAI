...
export default function ConversationInspector({ open, agentA, agentB, onClose }) {
  ...
  const messages = [
    {
      from: agentA.label.replace(' Agent', ''),
      to: agentB.label.replace(' Agent', ''),
      text: `Task: inspect intent "${agentB.label.replace(' Agent', '')} request" and dispatch`,
      dir: 'to',
    },
    {
      from: agentB.label.replace(' Agent', ''),
      to: agentA.label.replace(' Agent', ''),
      text: 'Acknowledged. Confidence 0.93. Querying memory…',
      dir: 'from',
    },
    {
      from: agentA.label.replace(' Agent', ''),
      to: agentB.label.replace(' Agent', ''),
      text: 'Proceed with best available tool.',
      dir: 'to',
    },
    {
      from: agentB.label.replace(' Agent', ''),
      to: agentA.label.replace(' Agent', ''),
      text: 'Done in 412ms. Result grounded (0.94).',
      dir: 'from',
    },
  ];
        ...
        >
          <div className="cc-conv-head">
            ...
            <div className="cc-conv-stats">
              <span className="cc-conv-stat">
                <Timer size={11} /> 412ms
              </span>
              <span className="cc-conv-stat">
                <Coins size={11} /> $0.0009
              </span>
            </div>
            <button
              type="button"
              className="cc-conv-close"
              onClick={onClose}
            ...
            {tabs.map((t) => {
              ...
              return (
                <button
                  key={t.id}
                  type="button"
                  className={`cc-conv-tab ${tab === t.id ? 'is-active' : ''}`}
                  onClick={() => setTab(t.id)}
                >
