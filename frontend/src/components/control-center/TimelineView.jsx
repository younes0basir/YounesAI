      ...
      <div className="cc-tlview-head">
        <div>
          ...
          <p className="cc-panel-sub">
            Pipeline stages reached by the orchestrator — updated in real time
          </p>
        </div>
        {steps.length > 0 && (
      ...
      <div className="cc-tlview-list">
        {rows.map((s, i) => {
          ...
          return (
            ...
            >
              <span
                className="cc-tlview-icon"
                style={{ background: done ? s.color : '#f1f5f9', color: done ? '#fff' : '#94a3b8' }}
              >
                {done ? <CheckCircle2 size={13} /> : <Circle size={13} />}
              </span>
              <div className="cc-tlview-main">
                <span className="cc-tlview-label">{s.label}</span>
                <span className="cc-tlview-desc">
                  {done ? s.desc : 'Waiting for the orchestrator to reach this stage…'}
                </span>
              </div>
              <span className={`cc-tlview-state ${done ? 'is-done' : ''}`}>
