      ...
      <div className="h-14 flex items-center justify-between px-4 lg:px-6 max-w-[1560px] mx-auto">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <form onSubmit={doSearch} className="glass-search hidden lg:flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-slate-700 transition-all">
            ...
            <kbd className="hidden xl:inline text-[10px] font-medium text-slate-600 bg-white border border-slate-200 rounded px-1.5 py-0.5">↵</kbd>
          </form>
          ...
          <div className="relative ml-0.5" ref={menuRef}>
            ...
            {showUserMenu && (
              <div className="absolute right-0 top-full mt-2 w-52 surface-elevated py-1.5 z-50 animate-fade-up rounded-xl">
                ...
                <button onClick={() => { setShowUserMenu(false); nav('/settings') }} className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-slate-700 hover:bg-violet-50/70 hover:text-violet-800 rounded-lg transition-colors"><Settings size={15} /> Settings</button>
                <button onClick={doLogout} className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"><LogOut size={15} /> Sign out</button>
              </div>
            )}
          </div>
        </div>
