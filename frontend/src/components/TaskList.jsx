      ...
      {tasks.map((t) => (
        <div key={t.id} className="surface-interactive flex items-center justify-between gap-3 p-3 rounded-xl border border-slate-100 bg-white">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="font-medium text-slate-800 flex items-center gap-2">
                ...
                {t.is_favorite ? <Star size={13} className="fill-amber-400 text-amber-400 shrink-0" /> : null}
              </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {t.urgency ? <span className={`badge ${urgencyStyles[t.urgency] || 'badge-muted'}`}>U{t.urgency}</span> : null}
            {t.priority ? <span className="badge badge-priority">P{t.priority}</span> : null}
            {t.due_at ? <span className="text-xs text-slate-400 hidden sm:inline">{new Date(t.due_at).toLocaleDateString()}</span> : null}
          </div>
        </div>
      ))}
    </div>
