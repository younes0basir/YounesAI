    ...
    return (
      ...
      >
        ...
        {it.status && !badgeCount && (
          <span
            className={`w-2 h-2 rounded-full ${statusMap?.orchestrator ? 'bg-green-500' : 'bg-slate-300'} ring-2 ring-white shrink-0`}
    ...
    <div className="h-full flex flex-col min-h-0 p-3.5">
      <div className="flex-1 overflow-y-auto nice-scroll min-h-0 space-y-5">
        ...
        <div>
          ...
          <nav className="flex flex-col gap-0.5">
            <NavItems
              items={studioItems}
              statusMap={{ orchestrator: orchestratorActive }}
      ...
      <div className="shrink-0 pt-3 border-t border-slate-200/70 space-y-1">
        <button
          ...
          className="w-full flex items-center gap-2.5 nav-item text-slate-600"
        >
          <Bot size={15} className="shrink-0" />
          <span className="flex-1 text-left">Ask AI anything…</span>
          <kbd className="text-[10px] font-medium text-slate-500 bg-white border border-slate-200 rounded px-1.5 py-0.5 shrink-0">
            /
          </kbd>
        </button>
        ...
        <NavLink
          ...
          className={({ isActive }) => `nav-item ${isActive ? 'nav-item-active' : ''}`}
        >
          <span className="nav-icon bg-orange-50 text-orange-600">
