  ...
  return (
    <div className="min-h-screen app-bg text-slate-800">
      ...
      <div className="shell-layout">
        {sidebarOpen && (
          <div
            className="fixed inset-0 bg-slate-900/30 backdrop-blur-sm z-20 xl:hidden"
            onClick={() => setSidebarOpen(false)}
          />
        )}

        <aside
          className={`sidebar-shell fixed xl:sticky top-14 z-20 xl:z-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full xl:translate-x-0'}`}
        >
...
function RightSidebar() {
  ...
  const unread = Array.isArray(reminders)
    ? reminders.filter((r) => !r.is_read && !r.dismissed_at).slice(0, 5)
    : [];
    ...
    <>
      <div className="surface rail-panel">
        <div className="rail-header">
          <h3>Today</h3>
          <div className="nav-icon bg-blue-50 text-blue-600">
            <Calendar size={14} />
          </div>
        </div>
      ...
      <div className="surface rail-panel">
        <div className="rail-header">
          ...
          <div className="nav-icon bg-amber-50 text-amber-600">
            <Bell size={14} />
          </div>
        </div>
        {unread.length === 0 ? (
        ...
        {reminders && Array.isArray(reminders) && reminders.length > 0 && (
          <button onClick={() => nav('/reminders')} className="rail-link">
            View all reminders
          </button>
        )}
      </div>
