...
export default function Login() {
  ...
  const error =
    ...
    (login.error ? 'Connection error — is the server running?' : null);

  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="hidden lg:flex auth-gradient text-white p-12 flex-col justify-between">
        <div>
          <div className="w-11 h-11 rounded-xl bg-white/10 flex items-center justify-center font-bold text-lg">
            AI
          </div>
      ...
      <div className="flex items-center justify-center p-6 sm:p-10">
        <form
          onSubmit={submit}
          className="w-full max-w-sm surface-elevated rounded-2xl p-8 space-y-5 animate-fade-up"
        >
          <div className="text-center lg:text-left">
            <div className="w-10 h-10 lg:hidden mx-auto rounded-xl bg-gradient-to-br from-violet-500 to-indigo-500 flex items-center justify-center text-white font-bold shadow-md">
              AI
            </div>
          ...
          {sessionExpired ? (
            <div className="flex items-start gap-2.5 bg-amber-50 text-amber-800 text-sm rounded-xl px-3.5 py-2.5 border border-amber-100">
              <AlertCircle size={16} className="mt-0.5 shrink-0" />
              <span>
                Your session expired or was issued by a different backend. Sign in again for{' '}
                {BACKEND_MODES[backendMode].label} ({BACKEND_MODES[backendMode].url}).
              </span>
            </div>
          ) : null}
