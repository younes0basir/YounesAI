...
function StatPill({ icon: Icon, value, label, onClick, tone }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-2xl border border-slate-200/60 bg-white/70 hover:bg-white hover:border-violet-200 hover:shadow-md backdrop-blur-sm transition-all text-left"
    >
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 animate-fade-up">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            {format(new Date(), 'EEEE, MMMM do')} · here's what's happening today.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 animate-fade-up">
        ...
        <StatPill
          icon={Sparkles}
          value={Array.isArray(activity) ? activity.length : 0}
          label="AI actions"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <section className="section-block">
            ...
            {tasksError ? (
              ...
              <EmptyState
                ...
                action={
                  <button onClick={() => nav('/tasks')} className="btn btn-secondary text-sm">
          ...
          <section className="section-block">
            ...
            {eventsError ? (
              ...
              <EmptyState
                ...
                action={
                  <button onClick={() => nav('/events')} className="btn btn-secondary text-sm">
              ...
              <div className="space-y-2">
                {todayEvents.map((e) => (
                  <button
                    ...
                    type="button"
                    onClick={() => nav('/events')}
                    className="w-full flex items-center gap-3 p-3 rounded-2xl border border-slate-200/50 hover:border-violet-200 hover:bg-violet-50/40 bg-white/60 backdrop-blur-sm transition-all text-left"
                  >
                    <div
                      className="w-1 h-10 rounded-full shrink-0"
                      style={{ backgroundColor: e.color || '#10b99a' }}
                    />
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-slate-800">{e.title}</div>
                      <div className="text-xs text-slate-400">
                        {new Date(e.starts_at).toLocaleTimeString('en', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </div>
        ...
        <section className="section-block">
          ...
          <button onClick={() => nav('/agents')} className="rail-link mt-1">
            Open AI Studio for full history
          </button>
        </section>
      </div>
    </div>
