...
function getFuzzyScore(text, query) {
  ...
  queryTokens.forEach((token) => {
    ...
    else if (textTokens.some((item) => item.startsWith(token) || token.startsWith(item)))
      score += 70;
...
function rankSearchResults(results, query, referencePoint) {
  const center =
    Array.isArray(referencePoint) && referencePoint.length === 2
      ? referencePoint
      : [31.7917, -7.0926];
  ...
  return results
    .map((result) => {
      ...
      const proximityBonus = hasCoords
        ? Math.max(0, 60 - haversineDistanceKm(center, [lat, lon]) / 20)
        : 0;
      const moroccoBonus =
        /morocco|maroc|rabat|casablanca|marrakech|tanger|fes|agadir|meknes|oujda|tetouan|kenitra|sale|temara|taza|khouribga|settat|berrechid|el jadida|safi|laayoune|dakhla/i.test(
          candidateText
        )
          ? 220
          : 0;
...
function buildSearchQueries(query) {
  ...
  const variants = new Set([
    ...
    `${baseQuery} morocco`,
    `${baseQuery} maroc`,
    `${baseQuery} in morocco`,
    `${baseQuery} in maroc`,
  ]);

  if (
    /cafe|coffee|hotel|restaurant|bar|bakery|university|school|hospital|pharmacy|supermarket|bank|gym|museum|park/i.test(
      baseQuery
    )
  ) {
...
export default function Places() {
  ...
  const runSearch = async (query, options = {}) => {
    ...
    try {
      ...
      const [lat, lon] =
        Array.isArray(mapCenter) && mapCenter.length === 2 ? mapCenter : MOROCCO_CENTER;
      ...
      const mapboxResponses = await Promise.allSettled(
        searchTerms.map((term) => {
          ...
          return fetch(
            `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(term)}.json?${params.toString()}`,
            {
      ...
      const geoapifyPayload =
        geoapifyResponse.status === 'fulfilled' ? await geoapifyResponse.value.json() : null;
      ...
      const fallbackResults =
        ranked.length > 0
          ...
          : [
              {
                display_name:
                  'No exact match found. Try a broader place name or address, or add Morocco to the search.',
              },
            ];
    ...
    <div className="space-y-5">
      ...
      <div className="surface-elevated p-5 space-y-4">
            ...
            {searchResults.map((result, index) => {
              ...
              return (
                <button
                  ...
                  disabled={isFallback}
                  className={`flex w-full items-start justify-between rounded-md px-2 py-2 text-left text-sm ${isFallback ? 'cursor-default text-gray-400' : 'hover:bg-gray-50'}`}
                >
        ...
        <div className="space-y-2">
          ...
          <a
            href={`https://www.geoapify.com/map?lat=${mapCenter[0]}&lon=${mapCenter[1]}&zoom=14`}
            target="_blank"
            rel="noreferrer"
...
function PlaceCard({ place: p, onToggle, onDelete, onFocus }) {
  return (
    <div className="surface surface-interactive rounded-2xl p-4 flex items-start gap-4 group">
      <button
        onClick={() => onFocus(p)}
        className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${p.is_visited ? 'bg-green-50 text-green-600' : 'bg-primary-50 text-primary-600'}`}
      >
        <MapPin size={18} />
      </button>
