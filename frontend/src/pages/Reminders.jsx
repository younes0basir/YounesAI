        ...
        <>
          {active.length > 0 && (
            <section className="space-y-3">
              <h3 className="text-xs font-semibold uppercase tracking-widest text-slate-400 px-1">
...
function ReminderCard({ reminder: r, onMarkRead, onDelete, onSnooze, onDismiss, muted = false }) {
  return (
    <div
      className={`surface surface-interactive p-4 flex items-start gap-4 group ${muted ? 'bg-slate-200/20' : ''}`}
    >
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${r.is_read || r.dismissed_at ? 'bg-slate-100 text-slate-400' : 'bg-amber-50 text-amber-600'}`}
      >
        <Bell size={18} />
      </div>
      <div className="flex-1 min-w-0">
        <div
          className={`font-semibold ${r.is_read || r.dismissed_at ? 'text-slate-500' : 'text-slate-900'}`}
        >
          {r.title}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mt-2">
          ...
          {r.warn_minutes_before ? (
            <span className="badge badge-pending">Voice {r.warn_minutes_before}min before</span>
          ) : null}
          {r.snoozed_until ? (
            <span className="badge badge-progress">
              Snoozed until {new Date(r.snoozed_until).toLocaleTimeString()}
            </span>
          ) : null}
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        {!r.dismissed_at ? (
          <>
            <button
              ...
              className="btn-icon hover:text-violet-600"
            ...
            <button
              ...
              className="btn-icon hover:text-amber-600"
              aria-label="Dismiss"
            >
              <CircleOff size={16} />
            </button>
        ...
        {!r.is_read ? (
          <button
            ...
            className="btn-icon hover:text-emerald-600"
