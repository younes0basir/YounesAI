    ...
    <div className="space-y-5">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {colors.map((c) => (
              <button
                ...
                className={`w-7 h-7 rounded-full border-2 transition-transform hover:scale-110 ${color === c ? 'border-violet-500 ring-2 ring-offset-2 ring-violet-300' : 'border-white shadow-sm'}`}
      ...
      {isError ? (
        ...
        <EmptyState
          icon={Calendar}
          title="No events yet"
          description="Create your first event to fill the calendar."
          action={
        ...
        <>
          <div className="space-y-3">
            {upcoming.map((e) => (
              <EventCard key={e.id} event={e} onDelete={(id) => setConfirmDeleteId(id)} />
            ))}
          </div>
          {past.length > 0 && (
    ...
    <div className="surface surface-interactive p-4 flex items-start gap-4 group">
      <div className="flex flex-col items-center w-12 shrink-0 rounded-2xl bg-gradient-to-b from-violet-50 to-primary-50 border border-violet-100/60 py-2">
        <span className="text-[10px] font-semibold text-slate-400 uppercase">
          {startDate?.toLocaleString('en', { month: 'short' })}
        </span>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-sm text-slate-500 mt-0.5">
          ...
          {endDate &&
            ` – ${isMultiDay ? `${endDate.toLocaleDateString()} ` : ''}${endDate.toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit' })}`}
        </div>
        {e.location_text ? (
          <div className="text-xs text-slate-400 mt-1">{e.location_text}</div>
        ) : null}
      </div>
      <button
        type="button"
