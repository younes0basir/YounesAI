...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    else if (gmail === 'error') toast.error(message ? decodeURIComponent(message) : 'Gmail connection failed')
  }, [searchParams])
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Settings" description="Your account and workspace preferences." kicker="Account" />
      ...
      <div className="surface p-5 max-w-xl space-y-6">
        <div className="section-block">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <span className="nav-icon bg-violet-50 text-violet-600"><User size={16} /></span>
              <div>
                <div className="text-sm font-semibold text-slate-800">{user?.display_name || 'Your account'}</div>
                <div className="text-xs text-slate-500">Display name</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <span className="nav-icon bg-blue-50 text-blue-600"><Mail size={16} /></span>
              <div>
          ...
          <div className="space-y-3">
                ...
                <div key={acc.id} className="flex items-center justify-between gap-3 p-3 rounded-xl bg-white/60 border border-slate-200/60">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">
                      ...
                      {st?.last_sync_at ? ` · Last sync ${new Date(st.last_sync_at).toLocaleString()}` : ''}
                    </div>
                    {st?.last_sync_error && (
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() => syncGmail.mutate(acc.id, {
                    ...
                    <button
                      ...
                      onClick={() => disconnectGmail.mutate(acc.id, {
            ...
            {accountList.length < 2 && (
              <button
                ...
                onClick={() => connectGmail.mutate(undefined, {
        ...
        <div className="section-block">
          ...
          <div className="flex items-center gap-3 text-sm text-slate-600">
            <span className="nav-icon bg-slate-100 text-slate-600"><Shield size={16} /></span>
