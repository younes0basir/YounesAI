...
export default function Files() {
  ...
  const handleDragLeave = (e) => {
    if (!dropRef.current?.contains(e.relatedTarget)) setIsDragOver(false);
  };
  ...
  const handleWebFolderSelect = async (e) => {
    ...
    setFolderOp({
      active: true,
      step: 'registering',
      message: `Preparing ${fileList.length} file(s) from folder...`,
    });
    try {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <PageHeader
          ...
          description="Manage local and indexed workspace documents."
        />
        <div className="flex items-center gap-2 self-start sm:self-auto">
          ...
          >
            {folderOp.active || isImportingFolder ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          {isError ? (
            ...
            <EmptyState
              icon={File}
              title={searchQuery || typeFilter ? 'No matches' : 'No files yet'}
              description={
                searchQuery || typeFilter
                  ? 'Try adjusting filters.'
                  : 'Select a local folder or upload a file to get started.'
              }
            />
          ) : (
            !isDragOver && (
              <div className="surface divide-y divide-slate-100 overflow-hidden">
                {processedFiles.map((f) => {
                  ...
                  return (
                    <div
                      key={f.id}
                      className="flex items-center gap-4 px-4 py-3.5 group hover:bg-violet-50/40 transition-colors"
                    >
                      <div
                        ...
                        <div className="flex items-center gap-2 text-xs text-slate-400 mt-0.5">
                          ...
                          {f.indexed_at ? (
                            <span className="hidden sm:inline-flex items-center gap-1">
                              <Clock size={11} />
                              {new Date(f.indexed_at).toLocaleDateString()}
                            </span>
                          ) : null}
                          {f.path && !searchQuery ? (
                            <span className="hidden sm:inline truncate text-slate-400 max-w-xs">
                              {f.path}
                            </span>
                          ) : null}
                        </div>
              ...
              {watchedFolders.map((folder) => {
                ...
                return (
                  <div key={folder.id} className="surface overflow-hidden">
                    ...
                    >
                      <div
                        className={`text-xs font-bold ${isExpanded ? 'text-indigo-500' : 'text-slate-400'} transition-transform ${isExpanded ? 'rotate-90' : ''}`}
                      >
                        ▶
                      </div>
                    ...
                    {isExpanded && folderFiles.length > 0 && (
                      <div className="divide-y divide-slate-50 border-t border-slate-100">
                        {folderFiles.map((f) => {
                          ...
                          const Icon = getFileIcon(ext);
                          const tone =
                            extTones[ext?.toLowerCase()] || 'bg-slate-100 text-slate-600';
                          return (
                            <div
                              key={f.id}
                              className="flex items-center gap-3 px-4 py-2.5 group hover:bg-violet-50/40 transition-colors"
                            >
                              <div
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          ...
          deleteFolder.mutate(folder.id, {
            onSuccess: (res) =>
              toast.success(
                `Removed "${folder.folder_path.split(/[\\/]/).pop()}" and ${res.deletedFiles} file(s)`
              ),
