...
import { buildIntelStats } from '../components/control-center/intelStats';
import {
  BarChart,
  Bar,
  XAxis,
...
const agentDefs = [
  {
    ...
    label: 'Orchestrator',
    icon: Brain,
    desc: 'Analyzes intent, routes & synthesizes responses',
  ...
  },
  {
    ...
    label: 'Task Agent',
    icon: CheckCircle,
    desc: 'Create, update, delete & list tasks',
  ...
  },
  {
    ...
    label: 'Event Agent',
    icon: Clock,
    desc: 'Calendar events & scheduling',
  ...
  },
  {
    ...
    label: 'Place Agent',
    icon: MapPin,
    desc: 'Addresses, locations & coordinates',
  ...
  },
  {
    ...
    label: 'File Agent',
    icon: FileText,
    desc: 'Document analysis & file operations',
  ...
  },
  {
    ...
    label: 'Memory Agent',
    icon: Database,
    desc: 'Semantic search & persistent storage',
  ...
  },
  {
    ...
    label: 'General Agent',
    icon: HelpCircle,
    desc: 'General conversation, greetings, fallback chat, and everyday Q&A',
  ...
  },
  {
    ...
    label: 'Desktop Agent',
    icon: HardDrive,
    desc: 'Local folder scanning, file discovery, desktop indexing and native file ops',
  ...
  },
  {
    ...
    label: 'Image Agent',
    icon: Image,
    desc: 'Text-to-image generation via NVIDIA FLUX.2 Klein',
  ...
  },
  {
    ...
    label: 'Email Agent',
    icon: Mail,
    desc: 'Gmail inbox classification, archiving, summarization, and task creation',
  ...
  },
  {
    ...
    label: 'Gemma Agent',
    icon: Sparkles,
    desc: 'Advanced reasoning, synthesis and deeper fallback analysis',
...
export default function Agents() {
  ...
  const {
    ...
    queryFn: () =>
      api
        .get('/agents/status', { params: timeFilterHours ? { hours: timeFilterHours } : {} })
        .then((r) => r.data),
    refetchInterval: 5000,
  });

  // Benchmark data
  const { data: benchmarkData, isLoading: benchmarkLoading } = useAgentBenchmark(
    timeFilterHours || 24
  );
  ...
  const { data: retrievalData } = useQuery({
    ...
    queryFn: () =>
      api.get('/monitoring/retrieval-stats', { params: { hours: 24 } }).then((r) => r.data),
    refetchInterval: 15000,
  });
  ...
  const usageStorageKey = userId ? `${getBackendURL()}:${userId}` : null;
  ...
  const usageTotals = useMemo(
    ...
    [benchmarkData, summaryData, metrics, agentsLive]
  );

  const intelStats = usageTotals;
  ...
  return (
    <div className="space-y-5 pb-8 animate-fade-up">
        ...
        {[
          {
            ...
            desc:
              metrics.length === 0
                ? 'No pipeline calls in this window yet'
                : `${successRate}% of calls succeeded`,
            icon: HeartPulse,
          ...
          },
          {
            label: 'Groundedness',
            value:
              evaluationSummary?.data?.avg_groundedness != null
          ...
          return (
            <div key={stat.label} className="agents-stat-card" title={stat.hint}>
              <div className="agents-stat-head">
                ...
                <span className="agents-stat-icon">
                  <Icon size={15} />
                </span>
              </div>
      ...
      <div className="agents-tab-help">
        ...
        <p>
          An immersive, living view of the whole agent ecosystem. Hover a node for live status,
          click to zoom in, press <strong>Space</strong> for the command palette, and follow the AI
          pipeline at the bottom of the stage.
        </p>
      </div>
              ...
              <tbody>
                {summaryData.summary.map((row, i) => {
                  ...
                  return (
                    <tr
                      key={i}
                      className="border-b border-slate-100/60 last:border-0 hover:bg-slate-50/40"
                    >
                      <td className="px-4 py-3 flex items-center gap-2">
                        <span
                          className={`w-6 h-6 rounded-lg flex items-center justify-center ${def?.bg || 'bg-slate-100'} ${def?.text || 'text-slate-500'}`}
                        >
                          <Icon size={13} />
                        </span>
                        <span className="font-semibold text-slate-800">
                          {def?.label || row.agent_name}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="badge badge-muted">{row.provider}</span>
                      </td>
                      <td className="px-4 py-3 font-bold text-slate-700">{row.total_calls}</td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded font-bold text-xs ${row.total_calls > 0 && row.success_count / row.total_calls > 0.8 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}
                        >
                          {row.total_calls > 0
                            ? Math.round((row.success_count / row.total_calls) * 100)
                            : 0}
                          %
                        </span>
                      </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="surface p-5">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart
                data={summaryData.summary.map((r) => ({
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart
                data={summaryData.summary.map((r) => ({
                  name: agentDefs.find((a) => a.name === r.agent_name)?.label || r.agent_name,
                  success:
                    r.total_calls > 0 ? Math.round((r.success_count / r.total_calls) * 100) : 0,
                }))}
              >
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                <XAxis dataKey="name" tick={{ fontSize: 11 }} tickLine={false} axisLine={false} />
          ...
          <div className="space-y-1">
            {recentCalls.map((m, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-50 transition-colors text-sm"
                >
                  <span
                    className={`flex items-center justify-center w-7 h-7 rounded-lg shrink-0 ${def?.bg || 'bg-slate-100'}`}
                  >
                    <Icon size={13} className={def?.text || 'text-slate-500'} />
                  </span>
                  <span className={`text-[13px] font-bold text-slate-800 w-20 shrink-0`}>
                    {def?.label || m.agent_name}
                  </span>
                      ...
                      <div
                        ...
                        style={{
                          ...
                          background: m.success
                            ? `linear-gradient(90deg, ${def?.color || '#64748b'}, ${def?.color || '#64748b'}88)`
                            : '#ef4444',
                        }}
                      />
                    </div>
                    <span className="text-[11px] text-slate-400 w-12 text-right shrink-0 font-semibold">
                      {m.latency_ms}ms
                    </span>
                  </div>
                  <span
                    className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded font-bold ${m.success ? 'text-emerald-700 bg-emerald-50' : 'text-red-700 bg-red-50'}`}
                  >
                    {m.success ? 'SUCCESS' : 'FAILURE'}
                  </span>
                  <span className="text-[11px] text-slate-400 w-16 text-right shrink-0 font-semibold">
                    {m.provider || '-'}
                  </span>
                </div>
              );
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="surface p-5 space-y-4">
          ...
          <p className="text-xs text-slate-500 -mt-2">
            Files that were scanned and split into searchable text chunks.
          </p>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 rounded-xl p-3">
              ...
              <div className="text-xl font-bold text-slate-700 mt-0.5">
                {indexData?.data?.stats?.unique_files || 0}
              </div>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <span className="text-[10px] font-bold uppercase tracking-wide text-slate-400">
              ...
              <div className="text-xl font-bold text-slate-700 mt-0.5">
                {indexData?.data?.stats?.total_chunks || 0}
              </div>
            </div>
          </div>
        </div>
        <div className="surface p-5 space-y-4">
          ...
          <p className="text-xs text-slate-500 -mt-2">
            Entities and the connections between them, extracted from your files.
          </p>
          <div className="grid grid-cols-2 gap-3">
        ...
        <div className="surface p-5 space-y-4">
          ...
          <p className="text-xs text-slate-500 -mt-2">
            How often retrieval finds a useful document when you ask a question.
          </p>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-emerald-50/50 rounded-xl p-3">
              ...
              <div className="text-xl font-bold text-emerald-700 mt-0.5">
                {retrievalData?.totals?.avg_ms || 0}ms
              </div>
            </div>
          </div>
        </div>
    ...
    <section className="agents-panel-enter">
      <div className="surface p-5">
        ...
        <div className="agents-tab-help mb-5">
          <HelpCircle size={14} className="shrink-0" />
          <p>
            <strong className="text-slate-700">How to read this:</strong> higher success rate,
            groundedness, and speed are better; lower hallucination, error rate, and cost-per-call
            are better.
          </p>
        </div>
                ...
                {benchmarkData.benchmark.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3 flex items-center gap-2">
                        <span
                          className={`w-7 h-7 rounded-lg flex items-center justify-center ${def?.bg || 'bg-slate-100'} ${def?.text || 'text-slate-500'}`}
                        >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded font-bold text-xs ${row.success_rate >= 90 ? 'bg-emerald-50 text-emerald-700' : row.success_rate >= 70 ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700'}`}
                        >
                          {row.success_rate}%
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded font-bold text-xs ${row.hallucination_rate <= 5 ? 'bg-emerald-50 text-emerald-700' : row.hallucination_rate <= 15 ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700'}`}
                        >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded font-bold text-xs ${row.error_rate <= 5 ? 'bg-emerald-50 text-emerald-700' : row.error_rate <= 15 ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700'}`}
                        >
                          {row.error_rate}%
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded font-bold text-xs ${row.avg_groundedness >= 0.8 ? 'bg-emerald-50 text-emerald-700' : row.avg_groundedness >= 0.5 ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700'}`}
                        >
          ...
          <div className="space-y-6">
            <div className="bg-slate-50 rounded-xl p-5 border border-slate-200">
              <h4 className="font-bold text-slate-700 mb-4 text-sm">
                Success Rate & Latency by Agent
              </h4>
              <ResponsiveContainer width="100%" height={250}>
                <BarChart
                  data={benchmarkData.benchmark.map((r) => ({
                    name: agentDefs.find((a) => a.name === r.agent_name)?.label || r.agent_name,
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(value, name) => [
                      name === 'success' ? `${value}%` : `${value * 10}ms`,
                      name === 'success' ? 'Success Rate' : 'Latency',
                    ]}
                  />
                  <Legend />
                    ...
                    data={[
                      {
                        metric: 'Success Rate',
                        value:
                          benchmarkData.benchmark.reduce(
                            (sum, r) => sum + (r.success_rate || 0),
                            0
                          ) / benchmarkData.benchmark.length,
                        fullMark: 100,
                      },
                      {
                        metric: 'Groundedness',
                        value:
                          benchmarkData.benchmark.reduce(
                            (sum, r) => sum + (r.avg_groundedness || 0) * 100,
                            0
                          ) / benchmarkData.benchmark.length,
                        fullMark: 100,
                      },
                      {
                        ...
                        value:
                          benchmarkData.benchmark.reduce(
                            (sum, r) => sum + Math.max(0, 100 - (r.avg_latency_ms || 0) / 10),
                            0
                          ) / benchmarkData.benchmark.length,
                        fullMark: 100,
                      },
                      {
                        ...
                        value:
                          benchmarkData.benchmark.reduce(
                            (sum, r) => sum + (1 - (r.error_rate || 0)) * 100,
                            0
                          ) / benchmarkData.benchmark.length,
                        fullMark: 100,
                      },
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-violet-50 rounded-xl p-4">
                ...
                <div className="text-xl font-bold text-violet-700 mt-1">
                  $
                  {(
                    benchmarkData.benchmark.reduce((sum, r) => sum + r.estimated_cost_usd, 0) /
                    benchmarkData.benchmark.reduce((sum, r) => sum + r.total_calls, 0)
                  ).toFixed(6)}
                </div>
              </div>
  ...
  return (
    <section className="surface p-6 agents-panel-enter">
      <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 max-w-xl">
            Send a prompt through the full orchestrator pipeline and inspect which agents were
            invoked.
          </p>
        </div>
        <button
      ...
      >
        ...
        <button
          type="submit"
          disabled={testChat.isPending || !message.trim()}
            ...
            <div className="flex items-center flex-wrap gap-2">
              ...
              {testResult.agents.map((name) => {
                ...
                return (
                  <span
                    key={name}
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold ${def?.bg || 'bg-slate-100'} ${def?.text || 'text-slate-600'}`}
                  >
        ...
        {newsLoading ? (
          ...
          <div className="space-y-3">
            {newsData.items.map((item) => {
              ...
              const badgeClass =
                categoryColors[item.category] || 'bg-slate-50 text-slate-700 border-slate-200';
              return (
