  ...
  return (
    <div className="space-y-5">
      ...
      {!normalized ? (
        ...
        <ErrorState title="Search failed" description="Could not complete your search. Please try again." onRetry={refetch} />
      ) : isLoading ? (
        <LoadingState message="Searching..." />
      ) : (
        <>
          <p className="text-sm text-slate-500 px-1">{total} result{total !== 1 ? 's' : ''} for <span className="font-semibold text-slate-700">&quot;{normalized}&quot;</span></p>
          <div className="grid gap-4 lg:grid-cols-3">
            <ResultCard section="tasks" title="Tasks" items={tasks} render={(item) => (
              <>
                <div className="font-medium text-slate-900">{item.title}</div>
                {item.description ? <div className="text-xs text-slate-500 mt-0.5">{item.description}</div> : null}
              </>
            )} />
            <ResultCard section="files" title="Files" items={files} render={(item) => (
              <>
                <div className="font-medium text-slate-900">{item.name}</div>
                {item.path ? <div className="text-xs text-slate-500 truncate mt-0.5">{item.path}</div> : null}
              </>
            )} />
            <ResultCard section="places" title="Places" items={places} render={(item) => (
              <>
                <div className="font-medium text-slate-900">{item.name}</div>
                {item.address ? <div className="text-xs text-slate-500 mt-0.5">{item.address}</div> : null}
              </>
            )} />
          </div>
        </>
  ...
  return (
    <div className="surface p-4">
      <div className="flex items-center gap-2 mb-3">
        <span className={`nav-icon ${meta.tone}`}><Icon size={14} /></span>
        <h3 className="text-sm font-semibold text-slate-800">{title} <span className="text-slate-400 font-normal">({items.length})</span></h3>
      </div>
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <div key={item.id} className="rounded-xl border border-slate-100 p-3 hover:border-primary-200 hover:bg-primary-50/30 transition-colors">
              {render(item)}
            </div>
          ))}
        </div>
